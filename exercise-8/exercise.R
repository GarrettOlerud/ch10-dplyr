...
pulitzer <- read.csv("data/pulitzer-circulation-data.csv", stringsAsFactors = F)
...
prize_change <- mutate(pulitzer, Pulitzer.Prize.Change = 
...
# 2004-2014?
prize_change %>% filter(Pulitzer.Prize.Winners.and.Finalists..2004.2014 == max(Pulitzer.Prize.Winners.and.Finalists..2004.2014)) %>% 
  select(Newspaper)
